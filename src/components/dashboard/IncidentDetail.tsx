...
function severityLabel(
  ...
  if (severity === 3) {
    return 'Moderate Priority Incident';
  }
...
export default function IncidentDetail() {
  ...
  const [
    incident,
    setIncident,
  ] = useState<IncidentDetailData | null>(
    null,
  );
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        setIncident({
          ...
          centroid:
            data.centroid &&
            typeof data.centroid ===
              'object' &&
            Number.isFinite(
              Number(data.centroid.lat),
            ) &&
            Number.isFinite(
              Number(data.centroid.lng),
            )
              ? {
                  lat: Number(
                    data.centroid.lat,
          ...
          telemetryCorroborated:
            data.telemetryCorroborated ===
            true,

          corroboratingGateId:
            typeof data.corroboratingGateId ===
            'string'
              ? data.corroboratingGateId
              : null,

          corroboratingGateDensityPct:
            data.corroboratingGateDensityPct !==
              null &&
            data.corroboratingGateDensityPct !==
              undefined &&
            Number.isFinite(
          ...
          assignedResponderId:
            typeof data.assignedResponderId ===
            'string'
      ...
      (snapshotError) => {
        ...
        setError(
          'Unable to synchronize incident details.',
        );
  ...
  const handleDispatchAndResolve =
    async (): Promise<void> => {
      if (!id || !incident) {
        return;
      }
      ...
      try {
        ...
        setDispatchError(
          'The incident could not be updated. Please retry before leaving this page.',
        );
      } finally {
          ...
          <Panel title="Explainable AI Assessment">
            ...
            {incident.telemetryCorroborated && (
              ...
              >
                ...
                >
                  {incident.corroboratingGateId
                    ? `Gate ${incident.corroboratingGateId}`
                    : 'Nearby gate'}{' '}
                  telemetry
                  independently supports
                  this incident assessment
                  {incident.corroboratingGateDensityPct !==
                  null
            ...
            Number.isFinite(
              ...
              <>
                ...
                >
                  <Metric
                    label="Latitude"
                    value={incident.centroid.lat.toFixed(
                      5,
                    )}
                  />
                  ...
                  <Metric
                    label="Longitude"
                    value={incident.centroid.lng.toFixed(
                      5,
                    )}
                  />
                </div>
                ...
                >
                  ...
                  fused incident location.
                  A responder route is shown
                  only when validated
          ...
          <Panel title="Responder Dispatch">
            {hasRealRoute ? (
              <>
                ...
                >
                  {incident.routingReason ??
                    'ETA calculated from the current operational routing model.'}
                </p>
              ...
              >
                ...
                >
                  Aegis Grid will not
                  display an unvalidated
                  travel time. ETA becomes
                  available when a
                  responder is assigned
                  and a valid venue route
                  has been calculated.
                </p>
              </div>
            )}
