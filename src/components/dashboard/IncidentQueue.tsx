...
const CATEGORY_LABELS: Record<string, string> = { medical: 'Medical', security: 'Security', lost_child: 'Lost Child', other: 'Other' };

function getCategoryLabel(category: string): string {
  return CATEGORY_LABELS[category] ?? category.replace(/_/g, ' ');
}

function getIncidentTitle(incident: QueuedIncident): string {
  if (incident.categories.length === 0) { return 'Operational Incident'; }
  if (incident.categories.length === 1) { return `${getCategoryLabel(incident.categories[0])} Incident`; }
  return 'Multi-Signal Incident';
}

function getReportSummary(incident: QueuedIncident): string {
  const meaningfulReport = incident.reports.find((report) => typeof report.text === 'string' && report.text.trim().length > 0);
  if (meaningfulReport?.text) { return meaningfulReport.text; }
  return incident.reasoningTrace;
}

function compareIncidents(first: QueuedIncident, second: QueuedIncident): number {
  /*
   * Severity dominates.
   *
...
export default function IncidentQueue() {
  ...
  useEffect(() => {
    const incidentQuery = query(collection(db, 'incidents'), where('status', '==', 'open'));

    return onSnapshot(
      incidentQuery,
      (snapshot) => {
        const nextIncidents = snapshot.docs
          .map((document): QueuedIncident => {
            const data = document.data();
            const reports = Array.isArray(data.reports) ? (data.reports as IncidentReport[]) : [];
            const categories = Array.isArray(data.categories) ? data.categories.filter((item): item is string => typeof item === 'string') : [];
            ...
            return {
              ...
              reasoningTrace: typeof data.reasoningTrace === 'string' ? data.reasoningTrace : 'Incident awaiting operational analysis.',
              reasoningSource: data.reasoningSource,
              status: typeof data.status === 'string' ? data.status : 'open',
              reportCount: Number(data.reportCount) || reports.length || 1,
              categories,
              reports,
      ...
      (snapshotError) => {
        console.error('Incident queue listener failed:', snapshotError);
        setError('Unable to synchronize the live incident queue.');
      },
    );
  }, []);
  ...
  return (
    <section aria-labelledby="triage-heading" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto', paddingRight: '0.5rem' }}>
      <h2 id="triage-heading" style={{ color: 'white', marginTop: 0, marginBottom: '0.75rem' }}>Active Triage Queue</h2>

      {error && (
        <div role="alert" style={{ background: '#3a1616', color: '#ffcdd2', padding: '1rem', borderRadius: '6px', marginBottom: '1rem' }}>
          {error}
        </div>
      )}
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                  ...
                  <time dateTime={new Date(incident.timestampMs).toISOString()} style={{ color: '#888', fontSize: '0.8rem', whiteSpace: 'nowrap' }}>
                ...
                {summary !== incident.reasoningTrace && (
                  <div style={{ marginTop: '0.75rem', padding: '0.75rem', background: '#171717', borderRadius: '6px' }}>
                    <strong style={{ display: 'block', color: '#aaa', fontSize: '0.75rem', textTransform: 'uppercase', marginBottom: '0.35rem' }}>AI reasoning</strong>
                    <span style={{ color: '#ccc', lineHeight: 1.45 }}>{incident.reasoningTrace}</span>
                  </div>
                )}
