export interface GateReading {
  ...
  /**
   * Optional operational context retained for compatibility
   * with existing backend callers.
   */
...
export function computeGateRisk(
  ...
  if (
    ...
    throw new RangeError(
      "previousCount must be >= 0",
    );
  }

  if (
    !Number.isFinite(secondsSinceLastReading) ||
    secondsSinceLastReading < 0
  ) {
    throw new RangeError(
      "secondsSinceLastReading must be >= 0",
    );
  }
  ...
  const densityPct =
    Math.min(100, Math.max(0, rawDensityPct));
